...
contract FlightSuretyData {
    ...
    modifier requireRegisteredAirline(address _airline) {
        require(
            RegisteredAirlines[_airline].isRegistered == true,
     ...
     * @return A bool
     */
    function isInsured(address passenger, string memory flight)
        public
        view
     ...
     *
     */
    function registerFirstAirline(address _airline)
        internal
        requireIsOperational
    {
        require(
            msg.sender == contractOwner,
            "Unauthorized to use this function"
        );
     ...
     *
     */
    function registerAirline(address _airline, address caller)
        external
        requireIsOperational
        ...
        returns (bool success)
    {
        ...
        require(
            !RegisteredAirlines[_airline].isRegistered,
    ...
    function buy(
        ...
        address sender,
        uint256 amount
    ) public requireIsOperational requireAuthorizedCaller {
        ...
        if (InsuredPassengers[passenger].isInsured == true) {
            ...
            InsuredPassengers[passenger] = Passenger({
                isInsured: true,
                isPaid: paid,
    ...
    function creditInsurees(string flight)
        ...
        requireAuthorizedCaller
    {
        address[] memory passengers = new address[](
            FlightPassengers[flight].length
        );
        uint index;
        uint amount = 0;
        ...
        for (uint i = 0; i < passengers.length; i++) {
            ...
            if (InsuredPassengers[passengers[i]].isPaid[index] == false) {
                ...
                amount = (InsuredPassengers[passengers[i]].insurancePaid[index])
                    .mul(15)
                    .div(10);
                InsurancePayment[passengers[i]] = InsurancePayment[
                    passengers[i]
                ].add(amount);
            }
        }
     ...
     *
     */
    function getFlightIndex(address pass, string memory flight)
        public
        view
        returns (uint index)
    {
        ...
        for (uint i = 0; i < flights.length; i++) {
            if (
                uint(keccak256(abi.encodePacked(flights[i]))) ==
                uint(keccak256(abi.encodePacked(flight)))
            ) {
                return (i + 1);
            }
        }
     ...
     *
     */
    function pay(address payee) external payable requireIsOperational {
     ...
     *
     */
    function fund(uint256 fundAmt, address sender)
        public
        requireAuthorizedCaller
        requireIsOperational
    {
     ...
     *
     */
    function getFlightsInsured(address passenger, string flight)
        external
        view
        requireIsOperational
        requireAuthorizedCaller
     ...
     *
     */
    function getFlightAmountInsured(string flight)
        external
        view
        requireIsOperational
        requireAuthorizedCaller
